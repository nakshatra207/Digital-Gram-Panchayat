...
import { OptimizedService } from '@/hooks/useOptimizedServices';
...
export const OptimizedServiceCard = memo<OptimizedServiceCardProps>(({ service, onApply }) => {
  const { t } = useLanguage();
  const theme = categoryThemes[service.category] || categoryThemes.certificates;
      ...
      <CardContent className="flex-1 space-y-4">
        <div className="bg-white/70 rounded-lg p-3 space-y-3 backdrop-blur-sm">
          <div className="flex justify-between items-center text-sm">
            ...
            <span className="font-semibold text-gray-900">{service.processing_time}</span>
          </div>
        ...
        <div className="bg-white/50 rounded-lg p-3">
          ...
          <div className="flex flex-wrap gap-1">
            {service.required_documents.slice(0, 2).map((doc, index) => (
            ...
            {service.required_documents.length > 2 && (
              <Badge variant="secondary" className="text-xs bg-white/80 text-gray-700">
                +{service.required_documents.length - 2} {t('more')}
              </Badge>
            )}
          </div>
        </div>
